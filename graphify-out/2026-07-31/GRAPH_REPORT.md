...
## Corpus Check
- 138 files · ~97,966 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1446 nodes · 3504 edges · 128 communities (78 shown, 50 thin omitted)
...
Cohesion: 0.07
Nodes (45): Config, Engine, DB, getEnv(), getEnvBool(), getEnvDuration(), getEnvInt(), getEnvList() (+37 more)
...
Cohesion: 0.07
Nodes (58): T, TestUserNetByExpense(), T, TestBuildLikePattern(), TestExpenseFilterLimit(), TestGroupMemberCountsAndNets(), TestListFriendExpensesFilters(), T (+50 more)
...
Cohesion: 0.21
Nodes (5): Context, Store, orDefault(), scanUser(), User
...
Cohesion: 0.18
Nodes (10): Authorization on the group route, Debt simplification, Direction, Prefill, Recording one, Reversing a settlement, Settle the whole group, Settlements (+2 more)
...
Cohesion: 0.20
Nodes (7): prefixCols(), trimSpace(), Context, Store, scanGroup(), TestCov2PureHelpers(), Group
...
### Community 23 - "New"
Cohesion: 0.13
Nodes (10): Service, Store, New(), todayISO(), Context, Context, Provider, covBank (+2 more)
...
### Community 32 - ".ImportFromSplitwise"
Cohesion: 0.39
...
Cohesion: 0.15
Nodes (17): Server, Request, ResponseWriter, absInt64(), Server, Request, ResponseWriter, decimals() (+9 more)

### Community 39 - "Background Scheduler"
Cohesion: 0.26
Nodes (12): Context, Duration, Service, New(), Service, T, newSchedService(), TestNew() (+4 more)

### Community 40 - ".UpdateAvatar"
Cohesion: 0.50
...
### Community 96 - "TestQueryWithout"
Cohesion: 0.33
Nodes (5): Transaction, Context, Service, itoa(), bankData
...
## Knowledge Gaps
- **121 isolated node(s):** `github.com/hafio/gosplit`, `ctxKey`, `collapseView`, `bankTxRow`, `balanceRow` (+116 more)
...
- **Why does `newHarness()` connect `body` to `TestGroupDetailPolish`, `seedDirectExpense`, `Server Wiring & Auth Middleware`, `Config, Mail & Bootstrap`, `TestListsRenderComponents`, `HTTP Integration Tests`, `nowISO`, `TestAdminSetPasswordRevokesTargetSession`, `TestExpenseFormRendersComponents`, `TestConvertExactBothAmounts`, `.PutBankData`, `TestExpenseNoteAndCollapse`, `TestFilterChipsRender`, `Web Helpers & Categories`, `.buildBatches`, `TestThemeColorUpdate`, `TestCov3BankConvertRedirect`?**
  _High betweenness centrality (0.139) - this node is a cross-community bridge._
- **Why does `New()` connect `New` to `.ImportFromSplitwise`, `.findOrCreateUser`, `User Service, Store & Auth`, `.CreateConversionExact`, `TestQueryWithout`, `Config, Mail & Bootstrap`, `Recurring Expenses`, `Service Layer Tests`, `Expense Service Logic`, `Currency Rate Providers`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Why does `Config` connect `Config, Mail & Bootstrap` to `.findOrCreateUser`, `Server Wiring & Auth Middleware`, `Bank Sync (Plaid)`, `New`?**
